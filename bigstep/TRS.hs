...
readPeano :: Term -> Maybe Int
readPeano (F "0" []) = Just 0
readPeano (F "s" [t])
  | Just n <- readPeano t = Just (n + 1)
readPeano _ = Nothing

readTermList :: Term -> Maybe [Term]
readTermList (F "nil" []) = Just []
readTermList (F "cons" [t, u])
  | Just us <- readTermList u = Just (t : us)
readTermList _ = Nothing

instance Show Term where
  show (V x)    = x
  show t
    | Just n <- readPeano t = show n
    | Just us <- readTermList t = 
       "[" ++ intercalate "," [ show u | u <- us ] ++ "]"
...
findMatch :: TRS -> Term -> Maybe (Term, Subst) -- ルールと項を受け取り、マッチするならば、その右辺と代入を返す
findMatch [] _ = Nothing -- 空のルールとマッチしない
findMatch ((l, r) : rules) t = -- ルールの先頭とマッチするか試す
    case match l t of -- lとtがマッチするならば、その代入sigmaを得る
        Just sigma -> Just (r, sigma) -- 右辺と代入を返す
        Nothing    -> findMatch rules t -- そうでなければ、残りのルールで試す

phi :: TRS -> Term -> Term -- ルールと項を受け取り、項を規約で可能な限り書き換えたものを返す
phi _ (V x) = V x -- 変数はそのまま
phi rules (F f t) = -- 関数項の場合
    let t' = map (phi rules) t -- まず引数をそれぞれ書き換え
        t'' = F f t' -- 書き換えた引数で関数項を再構成
    in case findMatch rules t'' of -- 書き換えた関数項とルールがマッチするか試す
        Just (r, sigma) -> phi rules (substitute r sigma) -- マッチしたならば、右辺を代入で書き換えたものを更に規約で書き換える
        Nothing         -> t'' -- マッチしなかったならば、そのまま返す

star :: TRS -> Term -> Subst -> Term -- ルールと項と代入を受け取り、項を代入で書き換えたものを規約で可能な限り書き換えたものを返す
star rules (V v) sigma = phi rules (substitute (V v) sigma) -- 変数の場合は、代入で書き換えたものを規約で書き換える
star rules (F f t) sigma = -- 関数項の場合
    let tNF = map (\ti -> star rules ti sigma) t -- まず引数をそれぞれ代入で書き換えたものを規約で書き換え
        t' = F f tNF -- 書き換えた引数で関数項を再構成
    in case findMatch rules t' of -- 書き換えた関数項とルールがマッチするか試す
        Just (r, tau) -> star rules r tau -- マッチしたならば、右辺を代入で書き換えたものを更に規約で書き換える
        Nothing       -> phi rules (substitute t' sigma) -- マッチしなかったならば、代入で書き換えたものを規約で書き換える

nf :: TRS -> Term -> Term
nf rules t = star rules t []
